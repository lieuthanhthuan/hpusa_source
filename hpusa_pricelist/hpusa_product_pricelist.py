...
class product_pricelist_version(osv.osv):
    ...
    _columns = {
        'min_amount': fields.float('Min Amount'), # hpusa configure
        'max_amount': fields.float('Max Amount'), # hpusa configure
    }
    def price_get_multi(self, cr, uid, pricelist_ids,version, products_by_qty_by_partner, context=None):
...
#            raise osv.except_osv(_('Warning!'), _("At least one pricelist has no active version !\nPlease create or activate one."))
        ...
        for product_id, qty, partner in products_by_qty_by_partner:
            for pricelist_id in pricelist_ids:
                ...
                cr.execute(
                    ...
                    'ORDER BY sequence',
                    (tmpl_id, product_id) + partner_args + (version, qty))
    ...
    def hpusa_price_get(self, cr, uid, ids, prod_id, qty,pricelist_version, partner=None, context=None):
        res_multi = self.price_get_multi(cr, uid, pricelist_ids=ids, version=pricelist_version, products_by_qty_by_partner=[(prod_id, qty, partner)], context=context)
