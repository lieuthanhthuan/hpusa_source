...
#     def log(self, message, level=openerp.netsvc.Logger()):
#         """
#         Helper method used print debug messages
#         """
#         openerp.netsvc.Logger().notifyChannel('pxgo_openffice_reports', level, message)
    ...
    def __init__(self, name, cr, uid, ids, data, context):
        ...
        self.pool = openerp.pooler.get_pool( cr.dbname )
    ...
    def execute(self, output_format='pdf', report_file_name=None):
        ...
        def _get_attachments():
            ...
            def get_attachments_func(browse_object):
                ...
                """
                db, pool = openerp.pooler.get_db_and_pool(self.dbname)
                cr = db.cursor()
    ...
    def process_template(self, template_file, output_format, context=None):
        ...
        """
        data ={}
        print template_file,self.openoffice_port,self.autostart_openoffice
        if context is None: context = {}
    
        template = oo_template.OOTemplate(source=None,
...
class openoffice_report(openerp.report.interface.report_int):
    """
    ...
    def __init__(self, name, model, parser=None, context=None):
        ...
        if not name.startswith('report.'):
            name = 'report.' + name
        if name in openerp.netsvc.Service._services:
    ...
    def create(self, cr, uid, ids, datas, context=None):
        ...
        #
        reports = openerp.pooler.get_pool(cr.dbname).get( 'ir.actions.report.xml' )
...
if 'old_create' not in dir(openerp.report.report_sxw.report_sxw):
    openerp.report.report_sxw.report_sxw.old_create = openerp.report.report_sxw.report_sxw.create 
...
openerp.report.report_sxw.report_sxw.create = create
